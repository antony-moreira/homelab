resource "proxmox_vm_qemu" "portainer" {
  count = local.portainer.count

  target_node = local.proxmox_node
  vmid        = local.portainer.vmid_prefix + count.index
  name = format(
    "%s-%s",
    local.portainer.name_prefix,
    count.index
  )
  ...
  cores   = local.portainer.cores
  sockets = local.portainer.sockets
  memory  = local.portainer.memory
  ...
  ipconfig0 = format(
    ...
    cidrhost(
      local.cidr,
      local.portainer.network_last_octect + count.index
    ),
  ...
  disk {
    ...
    size    = local.portainer.disk_size
    slot    = local.disks.main.slot
  }

  tags = local.portainer.tags
